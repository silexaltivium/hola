...
{ 
  ...
  if($err)
  	$db_errors[] =  $err;	
		...
		fclose($file);	 
		//UPLOADS TABLE CREATE
		$sql = "ALTER TABLE uploads ADD folder varchar (11), ADD shorturl text;";
		mysql_query($sql);
		$err = mysql_error();
		if($err)
			$db_errors[] =  $err;		
		...
		$current_space = 0;				
		//**********************
		// Create meta entries for current exisitng users
		//**********************
		//get all existing users
		$query = "SELECT id,space FROM users";
		...
		(00000000003, 'space', '$current_space', NULL);";
		mysql_query($sql);
		$err = mysql_error();
		if($err)
			$db_errors[] =  $err;
 //add meta entries for exising users
 if($users) 
 {
  foreach($users as $user)
  {
   $meta_down = "downloads_".$user['id'];
   $meta_up = "uploads_".$user['id'];
   $user_id = $user['id'];
   $meta_query = "INSERT INTO meta VALUES(NULL,'$meta_down',0,'$user_id')";
   mysql_query($meta_query);
   $meta_query = "INSERT INTO meta VALUES(NULL,'$meta_up',0,'$user_id')";
   mysql_query($meta_query);
  }
 }
...
<body id="install">
 ...
 <div id="main" class="notice container">
 	<div class="message info" style="display: block;">Continue this setup if you are upgrading from Version 1 only.</div>
 	<!--Wrapper-->
	 <div id="wrapper">
		  ...
		  <?php if($step == 1) : ?>
		   ...
		   <form method="post" id="install">
		    <p>
		    	...
		    	<input type="text" name="dbhost" value="<?php echo $config['dbhost'] ?>">
		    </p>	   	
		   	<p>
		    ...
		    <p>
		    	...
		    	<input type="text" name="dbuser" value="<?php echo $config['dbuser'] ?>">
		    </p>
		    
		    <p>   
		    	<label for="dbpass">Database Password:</label>
		    	<input type="text" name="dbpass" value="<?php echo $config['dbpass'] ?>">
		    </p>    
		    
		    <label for"apppath">Application Path:</label>
		    <input type="text" name="apppath" value="<?php echo $config['apppath'] ? $config['apppath'] : $dirs; ?>">
